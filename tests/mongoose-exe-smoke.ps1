...
Assert-True ($version.Output -match "mongoose 0.7.1") "mongoose.exe --version did not report expected version."
...
Assert-True ($statePaths.version -eq "0.7.1") "mongoose state did not report CLI version."
if ($env:GITHUB_REF_TYPE -eq "tag" -and $env:GITHUB_REF_NAME -match "^v") {
