...
Assert-True ($version.Output -match "mongoose 0.4.0") "installed mongoose.exe did not report expected version. Output: $($version.Output)"
...
Assert-True ($update.Output -match "Downloaded Mongoose 0.4.1; replacement will finish after this command exits.") "self-update did not report deferred replacement. Output: $($update.Output)"
...
$stagedDownloads = @(Get-ChildItem -Path $appRoot -Filter ".mongoose.exe.v0.4.1.*.download")
